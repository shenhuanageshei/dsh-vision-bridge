 ...
 *   AbortSignal into the request via AbortSignal.any, so turn cancellation
 *   aborts the in-flight HTTP request;
 * - **retry** with 2× jittered backoff on retryable failures (network errors,
 *   HTTP 429/5xx, engine-internal timeout aborts) — caller cancellation is
 *   NEVER retried and rethrows immediately;
