...
import { resolveModelInfoUnseamed } from './seam.js';
 ...
 *
 * @param ctx - plugin context (inject: agents for the modality gate,
 *   system-prompt; events ride ctx.on; llm/agentDefaultModel read optionally
 *   via ctx.get).
...
export function installAutoMode(ctx, runtime) {
  ...
  /** Whether the session's CURRENT model needs the vision bridge (design
   * §5.4 trigger: text-only model + new screenshot). Image-capable sessions
   * read screenshots natively — the bridge must stay silent. Unknown
   * model/modalities stay conservative (analyze) with a debug log.
   * @returns false when the model accepts images natively, true otherwise
   *   (text-only, undeclared, or unresolvable — the pre-gate behavior).
   */
  async function sessionModelNeedsBridge(sessionId) {
    try {
      const agent = ctx.agents?.get?.(sessionId);
      let selection;
      if (agent?.options?.provider && agent?.options?.model) {
        selection = { provider: agent.options.provider, model: agent.options.model };
      } else {
        // Agent without an explicit selection: the deployment default.
        const defaults = ctx.get?.('agentDefaultModel')?.currentSelection?.();
        if (defaults?.provider && defaults?.model) {
          selection = { provider: defaults.provider, model: defaults.model };
        }
      }
      if (!selection) {
        logger?.debug?.('vision-bridge auto: no model selection resolvable for session ' + sessionId + '; assuming the bridge is needed');
        return true;
      }
      // Optional read via ctx.get: the llm service is a core seam but not in
      // this plugin's strict inject list; its absence degrades to 'unknown'.
      // Unseamed on purpose (lib/seam.js): our own decoration injects image
      // capability into resolveModelInfo, and reading THAT answer here would
      // make every text-only session look image-capable — auto mode would then
      // skip the read and leave the screenshot unread (design §4.5/B5).
      const info = await resolveModelInfoUnseamed(ctx.get?.('llm'), selection.provider, selection.model);
      const modalities = info?.inputModalities;
      // Mirror the runtime projection gate (dsh-llm): absent modalities mean
      // unknown; an explicit list without 'image' is the text-only trigger;
      // 'image' present → the model reads natively.
      if (modalities === undefined) {
        logger?.debug?.('vision-bridge auto: model ' + selection.provider + '/' + selection.model + ' declares no input modalities; assuming the bridge is needed');
        return true;
      }
      return !modalities.includes('image');
    } catch (error) {
      logger?.debug?.('vision-bridge auto: model modality lookup failed (' + (error?.message ?? error) + '); assuming the bridge is needed');
      return true;
    }
  }
  ...
  const onSessionEvent = (session, event) => {
    ...
    for (const ref of capped) {
      ...
      // The entry is registered synchronously above this point (race
      // discipline); the model-modality gate below resolves asynchronously and
      // may settle the promise as 'skipped' WITHOUT ever starting a VLM call —
      // an image-capable session's screenshot is read natively by the model,
      // so the bridge must not analyze it (design §5.4: text-only + new
      // screenshot is the trigger).
      entry.promise = (async () => {
        try {
          if (await sessionModelNeedsBridge(session.id) === false) {
            entry.result = { status: 'skipped', gated: true };
            logger?.info?.('vision-bridge auto: session model accepts images natively; skipping bridge analysis for ' + shortId(ref.attachmentId));
            return entry.result;
          }
