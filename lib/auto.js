 ...
 *
 * @param ctx - plugin context (inject: agents, system-prompt).
...
export function installAutoMode(ctx, runtime) {
  ...
  const onSessionEvent = (session, event) => {
    ...
    for (const ref of capped) {
      const key = session.id + '::' + String(ref.attachmentId).toLowerCase();
      if (inflight.has(key)) continue; // same attachment re-registered: keep the first
      const controller = new AbortController();
      const entry = { seq, controller, result: undefined };
      entry.promise = (async () => {
        try {
  ...
  const onPreStep = async (payload, next) => {
    ...
    try {
      ...
      const waiters = [];
      for (const id of ids) {
        const entry = inflight.get(sessionId + '::' + id);
        if (entry) {
      ...
      // mirrors it. On timeout the step passes through and the note lands via
      // the PromptContext fallback once it settles.
  ...
  // PromptContext fallback: serves notes that settle AFTER their step already
  // assembled (timeout path). Consumed on first render — pruned so later steps
  // do not keep paying for it.
  const contextDisposer = ctx.systemPrompt.context({
    ...
    text: (assembly) => {
      ...
      return '<vision-context>\n' + UNTRUSTED_DISCLAIMER
        + '\nLate-arriving descriptions for image(s) sent earlier in this session:\n\n'
        + sections.join('\n\n') + '\n</vision-context>';
    },
  });
