...
`;
        ...
        const en = {
            ...
            credentialConflict: "Entry {name} already exists — pick another name (existing entries are never overwritten).",
        ...
        const zh = {
            ...
            credentialConflict: "条目 {name} 已存在——请改名（已有条目不会被覆盖）。",
        ...
        class VisionBridgeCardController {
            ...
            async save() {
                ...
                // refuses without overwriting; the key never lands in this
                // plugin's settings either way.
                if (this.credentialMode === "paste") {
                    const entry = this.pendingName.trim();
                    const key = this.pendingKey.trim();
                    if (key === "") {
                    ...
                    try {
                        const described = await service.describe([entry]);
                        if (described?.ok !== true) {
                            // A refused describe is NOT "unconfigured" (review #4):
                            // fall through to set only on an explicit
                            // configured === false answer; otherwise treat it
                            // as a write failure so an existing entry is never
                            // silently overwritten.
                            this.credentialError = { kind: "failed", message: "credential status check refused" };
                            this.publish();
                            return;
                        }
                        const view = described.value?.[entry];
                        if (view?.configured === true) {
                            this.credentialError = { kind: "conflict", name: entry };
                            this.publish();
                            return;
                        }
                    } catch (error) {
        ...
        function VisionBridgeCard(props) {
            ...
            function engineGroup() {
                ...
                function credentialErrorNote() {
                    if (state.credentialError.kind === "conflict") {
                        return e("span", { key: "cred-err", className: "dvb-hint dvb-hint-error" },
                            t("credentialConflict").replace("{name}", state.credentialError.name ?? ""));
                    }
            ...
            function envGroup() {
                ...
                function envRow(target) {
                    ...
                    const state3 = isAdmission ? admission?.status : undefined;
                    const conflict = !isAdmission && modlens?.conflict === true;
                    const unknownState = isAdmission ? state3 === "unknown" : modlens?.paste === "unknown";
                    const ok = isAdmission ? state3 === "ok" : !conflict && !unknownState;
                    const warn = isAdmission ? state3 === "missing" : conflict;
                    const label = isAdmission
                        ? (state3 === "missing" ? t("admissionMissing") : state3 === "unknown" ? t("admissionUnknown") : t("admissionOk"))
                    ...
                    if (warn && state.explainOpen[target]) {
                        parts.push(e("div", { key: "explain", className: "dvb-explain" },
                            isAdmission ? t("admissionExplain") : t("modlensExplain")));
                    }
