...
/** 0.1.7 declares which parts of a settings document a running plugin accepts
 * live (design §4-B): exactly the fields the settings card edits, so a card
 * write must reach the running plugin, while every unmarked field keeps taking
 * the loader's ordinary update path (a reload re-runs `apply()`).
 *
 * `vol()` probes the capability rather than importing it: a schemastery without
 * `.volatile()` — the pre-volatile generation — receives the plain schema, so
 * this one schema definition serves both generations.
 */
...
export const Config = z.object({
  ...
  mode: vol(z.union(['tool', 'auto', 'both']).default('both')),
  /** Runtime service seam (docs/design-v4-runtime-seam.md §4.3): decorating the
   * in-process `llm.resolveModelInfo` service method is what lets a text-only
   * model's image prompt through the admission gates. All three keys are live
   * (§4.4) — lib/index.js uninstalls, recomputes and reinstalls on a change. */
  seam: vol(z.object({
    /** auto = install the decoration; off = leave the running service alone. */
    mode: z.union(['auto', 'off']).default('auto'),
    /** `"provider:model"` routes to inject into; empty = every route whose
     * adapter declares text-only input. */
    include: z.array(z.string()).default([]),
    /** true = when the vision reader is not configured, do NOT inject: the
     * admission gate keeps rejecting, so a screenshot is never "sent" with
     * nobody able to read it (§4.5, B5). */
    requireReader: z.boolean().default(true),
  })),
  provider: vol(z.object({
  ...
  credential: vol(z.string().default('VISION_API_KEY')),
  /** Single VLM call budget in milliseconds. */
  timeoutMs: vol(z.number().default(60000)),
  /** In-flight VLM call cap. */
  concurrency: vol(z.number().default(2)),
  cache: z.object({
  ...
  visionCapabilities: vol(z.object({
  ...
  language: vol(z.union(['zh', 'en']).default('zh')),
   ...
   * fingerprint. */
  promptExtra: vol(z.string().default('')),
  autoMode: vol(z.object({
    /** Maximum images analyzed automatically per user/message event. */
    maxPerTurn: z.number().default(3),
  })),
});

/** `seam.include` entry syntax (design §4.3): exactly one `provider:model`
 * pair, DSH id alphabet. An invalid entry refuses the whole generation and the
 * previous configuration keeps serving. */
const SEAM_ROUTE_RE = /^[A-Za-z0-9._-]+:[A-Za-z0-9._-]+$/;
...
/** Max characters of promptExtra after trimming; the client half mirrors this
 * bound for its draft pre-validation (kept in sync by a cross-check test). */
...
export function resolveConfig(config = {}, options = {}) {
  ...
  const seamMode = config.seam?.mode ?? 'auto';
  if (seamMode !== 'auto' && seamMode !== 'off') {
    throw new TypeError('vision-bridge: seam.mode must be "auto" or "off"');
  }
  const seamIncludeRaw = config.seam?.include ?? [];
  if (!Array.isArray(seamIncludeRaw)) {
    throw new TypeError('vision-bridge: seam.include must be an array of "provider:model" strings');
  }
  const seamInclude = seamIncludeRaw.map((entry) => {
    const value = typeof entry === 'string' ? entry.trim() : '';
    if (!SEAM_ROUTE_RE.test(value)) {
      throw new TypeError('vision-bridge: seam.include entry ' + JSON.stringify(entry) + ' must match "provider:model"');
    }
    return value;
  });
  const seamRequireReader = config.seam?.requireReader ?? true;
  if (typeof seamRequireReader !== 'boolean') {
    throw new TypeError('vision-bridge: seam.requireReader must be a boolean');
  }

  return {
    mode,
    seam: { mode: seamMode, include: seamInclude, requireReader: seamRequireReader },
...
/**
 * Seam whitelist test (design §4.3): an empty list means every route, otherwise
 * the route must be listed verbatim as `provider:model`.
 *
 * @param include - resolved `seam.include` array.
 * @param provider - provider id of the route being resolved.
 * @param model - model id of the route being resolved.
 * @returns true when the seam may inject image capability for this route.
 */
export function seamRouteIncluded(include, provider, model) {
  if (!Array.isArray(include) || include.length === 0) return true;
  return include.includes(String(provider) + ':' + String(model));
}
