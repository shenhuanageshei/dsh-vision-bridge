...
import { installSeam, seamInstalled, seamMarker, seamTarget } from './seam.js';
...
/** N-I2: lifetime of one reader-availability verdict (see readerAvailable). */
export const READER_VERDICT_TTL_MS = 5000;

/** §4.7-1 log line: the ⓪ self-heal stood down because the seam is live. */
export const SEAM_LIVE_SKIP_SELF_HEAL_LOG = '[vision-bridge] seam live — admission disk patch not needed';

/** Process-lifetime seam liveness (§4.7-1). The ⓪ self-heal decision runs BEFORE
 * the ④ self-proof, so a cold start cannot know the verdict and performs the
 * (harmless) repair; a later fiber mount in the SAME process reuses it. `target`
 * pins the verdict to one llm instance — a different instance means the verdict
 * is stale and the repair runs again. Module-level on purpose: unlike the rest
 * of this plugin's state it must survive a fiber restart (§12.1 pattern). */
const processSeamState = { target: undefined, live: undefined, reason: 'unknown' };

/** Drop the process-lifetime seam verdict (tests only). */
export function resetProcessSeamState() {
  processSeamState.target = undefined;
  processSeamState.live = undefined;
  processSeamState.reason = 'unknown';
}
...
export async function apply(ctx, config = {}) {
  ...
  /** N-I2: how long one reader verdict is reused. `buildModelCatalog` asks the
   * same question once per provider×model, and every fresh answer costs a
   * credential resolution, so the verdict is cached briefly. The cache key is
   * the reader triple: a configuration change misses it immediately. */
  const readerVerdict = { key: '', at: 0, verdict: undefined };
  ...
  async function readerAvailable() {
    ...
    const key = baseURL + '\u0000' + model + '\u0000' + credential;
    const startedAt = Date.now();
    if (readerVerdict.verdict !== undefined && readerVerdict.key === key
      && startedAt - readerVerdict.at < READER_VERDICT_TTL_MS) {
      return readerVerdict.verdict;
    }
    const verdict = await computeReaderVerdict(baseURL, model, credential);
    readerVerdict.key = key;
    readerVerdict.at = startedAt;
    readerVerdict.verdict = verdict;
    return verdict;
  }

  async function computeReaderVerdict(baseURL, model, credential) {
  ...
  /** §4.7-1: publish the process-wide verdict after every re-seat or self-proof,
   * so a mode/include change (or a failed install) can never leave a stale
   * "live" standing the disk repair down. */
  function rememberSeamLiveness() {
    processSeamState.target = seamTarget(readSeamLlm());
    processSeamState.live = seam.live;
    processSeamState.reason = seam.reason;
  }

  /** N-B2-4/F3: a REAL residue — the mark is on the instance but the method it
   * holds is not the layer we installed (a stale mark, or another decorator on
   * top). "Installed but not self-proven" is NOT a residue: the mark and the
   * method agree there, only the behaviour proof is missing. */
  function seamResidueNow() {
    const llm = readSeamLlm();
    return seamMarker(llm) !== undefined && !seamInstalled(llm);
  }

  /** The process-wide verdict — but only while it belongs to THIS llm instance;
   * anything else reads as unknown, which runs the repair. */
  function processSeamLive() {
    if (processSeamState.target === undefined) return undefined;
    if (processSeamState.target !== seamTarget(readSeamLlm())) return undefined;
    return processSeamState.live;
  }

  /** (Re)seat the decoration for the CURRENT seamOptions. */
  function installCurrentSeam() {
    try {
      seatSeamForCurrentOptions();
    } finally {
      // Publish only a verdict we actually HAVE. A fresh install is "not
      // self-proven yet", and overwriting here would erase the previous mount's
      // proof before ⓪ of the next mount gets to read it (that proof is the
      // whole point of the process-lifetime record). mode=off and unsupported
      // ARE verdicts — there is no seam — so they publish live=undefined.
      if (seam.status !== 'installed' && seam.status !== 'already') rememberSeamLiveness();
    }
  }

  function seatSeamForCurrentOptions() {
    uninstallSeam();
  ...
  async function verifySeamLive() {
    try {
      await runSeamSelfProof();
    } finally {
      // Even a failed/timed-out proof republishes (live = undefined ⇒ the next
      // mount runs the disk repair again, never a stale skip).
      rememberSeamLiveness();
    }
  }

  async function runSeamSelfProof() {
  ...
  try {
    ...
    // unreached. Never throws and never blocks startup (§12.4-3), and runs at
    // most once per node_modules directory: the outcome is cached, so a fiber
    // re-entry reuses it instead of re-reading disk.
    //
    // §4.7-1: when the seam is already self-proven LIVE in this process, the
    // disk patch is redundant — the in-memory decoration is what lets the gate
    // through right now, and a disk write only affects the NEXT boot. The
    // decision is deliberately taken here (the order of this block is nailed by
    // the static assertions in tests/dual-gen.test.mjs): a cold start cannot
    // know the verdict yet — the self-proof runs at ④ — so it performs the
    // harmless repair, and the next mount of this process stands it down.
    // F2 (N-B2-1): the process verdict is inherited across mounts, but a mount
    // whose composition base switches the seam off must not borrow it — that
    // mount has no live seam, so the disk repair is needed again. Only the
    // base is knowable here (the settings face runs later, by design).
    const baseSeamOff = config?.seam?.mode === 'off';
    if (!baseSeamOff && processSeamLive() === true) {
      logger.info(SEAM_LIVE_SKIP_SELF_HEAL_LOG);
    } else {
      try {
    ...
    if (typeof ctx.inject === 'function') {
      ctx.inject(['webServer'], (scope) => installServerRoutes(scope, {
        ...
        // §4.7-2 / §4.6: the health panel reads the seam state (read-only) to
        // stand the runtime probe down and to render its own four rows.
        getSeamState: () => ({
          status: seam.status,
          live: seam.live,
          reason: seam.reason,
          mode: seamOptions.mode,
          include: seamOptions.include,
          requireReader: seamOptions.requireReader,
          // Read-only real-residue flag (F3/N-B2-4) — never inferred by the card.
          residue: seamResidueNow(),
        }),
        ...buildAdmissionProbeDeps(ctx, settingsReader.get),
      }));
    }

    // N-I4: the llm service can arrive AFTER this fiber mounted (a sibling
    // bundle may provide it later), and the seam would then stay uninstalled for
    // the whole process. One scoped inject retries the install + self-proof the
    // moment it shows up; with llm already available the guard makes it a no-op.
    // This is the second inject scope of this plugin — tests/dual-gen.test.mjs
    // pins the register list, so that assertion was widened with it (the
    // webServer scope is still requested exactly once, and is still first).
    if (typeof ctx.inject === 'function') {
      try {
        ctx.inject(['llm'], () => {
          if (seam.status === 'installed' || seam.status === 'already') return;
        ...
        logger.warn('vision-bridge: runtime seam could not register the late-llm retry (' + (error?.message ?? error) + ')');
      }
    }
