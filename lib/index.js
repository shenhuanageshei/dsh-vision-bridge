...
import { BAKED_PROVIDER_DEFAULTS, Config, SETTINGS_NAMESPACE, configFingerprint, resolveConfig } from './config.js';
...
export const inject = ['tools', 'settings', 'attachments', 'agents', 'system-prompt', 'credentials'];
 ...
 * drift in another plugin must not break us). Missing toolkit values fall back
 * to the baked defaults. The file write is atomic.
 */
function ensureFrozenDefaults(settings, file, logger) {
  if (existsSync(file)) return readFrozenDefaults(file, logger);
  let copied;
  try {
    ...
    logger?.warn?.('vision-bridge: first run — vision-toolkit namespace unreadable (' + (error?.message ?? error) + '); using baked defaults');
  }
  const frozen = copied ?? { ...BAKED_PROVIDER_DEFAULTS };
  try {
    mkdirSync(join(file, '..'), { recursive: true });
    const tmp = file + '.tmp';
    writeFileSync(tmp, JSON.stringify(frozen, null, 2), 'utf8');
    renameSync(tmp, file);
  } catch (error) {
...
export async function apply(ctx, config = {}) {
  ...
  async function resolveCredential(resolved) {
    let resolvedCredential;
    try {
      resolvedCredential = await ctx.credentials.resolve(credentialRef(resolved.provider.credential));
    } catch (error) {
      throw new Error('vision bridge: credential "' + resolved.provider.credential + '" could not be resolved (' + (error?.message ?? error) + '). Store it in the DSH credential provider or set the environment variable.');
    }
    if (!resolvedCredential?.value) {
      throw new Error('vision bridge: credential "' + resolved.provider.credential + '" is not configured. Store it in the DSH credential provider or set the environment variable, then retry.');
    }
  ...
  async function analyzeImage(ref, rawQuestion, { namespace, signal } = {}) {
    ...
    return state.dedup.run(key, () => state.gate.run(async () => {
      ...
      const apiKey = await resolveCredential(resolved);
      ...
      const result = await state.bridge.analyze({
        ...
        userRequest: (question ? question : '') + languageDirective,
      });
  ...
  const runtime = {
    ...
    analyzeImage,
    combinedSignal: (controller) => (lifecycle.signal ? AbortSignal.any([lifecycle.signal, controller.signal]) : controller.signal),
  };

  // ④ Tool mode.
  disposers.push(ctx.effect(() => ctx.tools.register(createVisionBridgeReadTool(runtime)), 'vision-bridge: vision_bridge_read tool'));

  // ⑤ Auto mode lands in M3 (lib/auto.js); mode=auto/both currently behaves as tool-only.
  if (state.config.mode !== 'tool') {
    logger.warn('vision-bridge: mode="' + state.config.mode + '" requests auto mode, which is not installed yet (M3); running tool-only.');
  }
  ...
  // ⑦ Disposer: abort lifecycle (in-flight auto analyses observe it), flush caches.
  return () => {
    for (const dispose of disposers.reverse()) {
      try { dispose(); } catch { /* disposal must not mask the rest */ }
    }
    lifecycle.abort();
    void state.toolCache?.close();
    void state.autoCache?.close();
  };
}
