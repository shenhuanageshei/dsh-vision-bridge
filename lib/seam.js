...
export function installSeam(llm, options = {}) {
  ...
  const ownDescriptor = Object.hasOwn(raw, 'resolveModelInfo')
    ...
    : undefined;
  /** N-I2: the last blocked route+reason that was announced. buildModelCatalog
   * calls resolveModelInfo once per provider×model, so an unconditional warn
   * would flood the log; the latch fires on the ALLOW→DENY transition only.
   *
   * KNOWN DEVIATION, registered on purpose (review N-B2-3, NOT fixed): the latch
   * is keyed by route+reason, so a batch walking N DIFFERENT text-only routes
   * still logs one line each. The expensive half of N-I2 — one credential
   * resolution per call — is already gone (READER_VERDICT_TTL_MS in
   * lib/index.js), so what remains is bounded by the number of text-only models
   * in the provider projection. Grouping routes would need a flush window and is
  ...
  async function inject(info, provider, model) {
    ...
    if (!verdict.allowed) {
      ...
      // hard rejection stays, and the reason is said out loud once per episode
      // (N-I2: the batch consumers would otherwise repeat it per model).
      const key = routeLabel(provider, model) + '|' + (verdict.reason ?? 'reader-unavailable');
      if (key !== blockedLogged) {
        blockedLogged = key;
    ...
    blockedLogged = undefined; // an allowed call re-arms the transition log
  ...
  // N-I3 + N-I6: a frozen / non-extensible service instance must degrade to the
  // documented three-state contract instead of throwing the whole plugin load
  // away. Both writes are guarded by ONE rollback, because the marker write
  // lands AFTER the method replacement: if it throws (an instance that is
  // non-extensible but already owns a configurable resolveModelInfo, so the
  // defineProperty succeeds), the replaced method has to be put back.
  try {
    ...
    if (raw.resolveModelInfo === wrapped) {
      try {
        if (ownDescriptor !== undefined) Object.defineProperty(raw, 'resolveModelInfo', ownDescriptor);
        else delete raw.resolveModelInfo;
      } catch { /* the instance refuses even the rollback; nothing further we can do */ }
    }
