 ...
 *   health panel (§11.3E); since §12 the admission item carries the disk
 *   marker state AND the runtime behavior probe's verdict;
...
/** Rejection reason the stock admission gate reports for image prompts on a
 * text-only model (dsh-api-session-controller/lib/index.js:751). */
export const ADMISSION_GATE_LIVE_REASON = 'MODEL_DOES_NOT_SUPPORT_IMAGES';

/** §4.7-2: the env route's third runtime state — the probe was deliberately NOT
 * submitted because the runtime seam is live (the probe would read 'dead'
 * regardless of the disk state, and would leave a synthetic user/message in the
 * scratch session, since this host exposes no message-deletion API). */
export const ADMISSION_PROBE_SKIPPED_REASON = 'seam-live';
/** Budget for one synthetic admission submission (§12.2 B) — the gate answers
 * before any model call, so a healthy probe settles in milliseconds. */
const ADMISSION_PROBE_TIMEOUT_MS = 15000;
...
/**
 * Read the admission verdict out of a failed synthetic submission (§12.2 B).
 * A RemoteError carries `code` + `details` (dsh-api-gateway/lib/client.js:17-33);
 * the gate's rejection is `session/attachment-invalid` with
 * `details.reason === 'MODEL_DOES_NOT_SUPPORT_IMAGES'`. Anything else —
 * `session/agent-busy`, transport failures, timeouts — is inconclusive and
 * must never be reported as a verdict.
 * @param {unknown} error - the caught submission failure.
 * @returns {'live'|'unknown'} 'live' only for the gate's own rejection.
 */
export function classifyAdmissionProbeError(error) {
  const reason = String(error?.details?.reason ?? error?.reason ?? error?.data?.reason ?? '');
  const message = String(error?.message ?? error);
  if (reason === ADMISSION_GATE_LIVE_REASON || message.includes(ADMISSION_GATE_LIVE_REASON)) return 'live';
  if (/does not support image input/i.test(message)) return 'live';
  return 'unknown';
}

/** Reject after `timeoutMs` so a wedged submission cannot hold the env route
 * open. The timer is cleared as soon as either side settles, so it never keeps
 * the process alive (and is deliberately NOT unref'd: an unref'd timer lets
 * the runtime exit before the race settles when nothing else holds the loop). */
function withProbeTimeout(promise, timeoutMs) {
  if (!Number.isFinite(timeoutMs) || timeoutMs <= 0) return promise;
  let timer;
  const timeout = new Promise((_resolve, reject) => {
    timer = setTimeout(() => reject(new Error('admission probe timed out after ' + timeoutMs + ' ms')), timeoutMs);
  });
  return Promise.race([promise, timeout]).finally(() => { clearTimeout(timer); });
}

/**
 * Runtime behavior probe of the admission gate (design §12.2 B): the disk
 * marker only says what is written, not what the loaded core module does
 * (§12.0-2), so the running gate is observed by submitting one synthetic
 * image prompt into a scratch session.
 *
 * - submission admitted → the loaded gate is lazified → `dead`;
 * - rejection carrying MODEL_DOES_NOT_SUPPORT_IMAGES → `live`;
 * - agent-busy, missing scratch session, transport failure, timeout, or no
 *   RPC face at all → `unknown` (never a guess — §12.4-4).
 *
 * The RPC face arrives through `deps.promptRemote`; this module never imports
 * a core package and never reaches for a live session itself, so the whole
 * probe is exercisable with stubs. A deployment that leaves `promptRemote`
 * unwired gets `unknown`, which the card renders as "written to disk, runtime
 * state unverified" rather than a false ✓.
 *
 * @param {object} deps - injected collaborators:
 *   `promptRemote({sessionId, content})` → the submission (absent ⇒ unknown),
 *   `ensureScratchSession()` → a session id whose current model is text-only
 *   (the gate only fires for text-only models), `cleanupScratchMessage
 *   (sessionId, result)` → best-effort removal of the synthetic message
 *   (§12.6-20; no deletion API exists, so a deployment may leave it out),
 *   `logger`, `probeTimeoutMs`.
 * @returns {Promise<{runtime: 'dead'|'live'|'unknown', reason?: string}>}
 */
export async function probeAdmissionGate(deps = {}) {
  const logger = deps.logger ?? { info() {}, warn() {}, error() {} };
  const promptRemote = deps.promptRemote;
  if (typeof promptRemote !== 'function') return { runtime: 'unknown', reason: 'rpc-face-unavailable' };

  let sessionId;
  try {
    sessionId = typeof deps.ensureScratchSession === 'function' ? await deps.ensureScratchSession() : undefined;
  } catch (error) {
    logger.warn?.('vision-bridge: admission probe could not prepare its scratch session (' + truncateText(error?.message ?? error) + ')');
    return { runtime: 'unknown', reason: 'scratch-session-unavailable' };
  }
  if (typeof sessionId !== 'string' || sessionId === '') return { runtime: 'unknown', reason: 'scratch-session-unavailable' };

  const content = [
    { type: 'text', text: PROBE_REQUEST },
    { type: 'image', mediaType: 'image/png', data: PROBE_PNG },
  ];
  try {
    const result = await withProbeTimeout(
      promptRemote({ sessionId, content }),
      deps.probeTimeoutMs ?? ADMISSION_PROBE_TIMEOUT_MS,
    );
    // Admitted: the running gate let an image prompt through. Drop the
    // synthetic message again when the deployment provides a removal seam
    // (best effort — a failed cleanup must not change the verdict).
    if (typeof deps.cleanupScratchMessage === 'function') {
      try {
        await deps.cleanupScratchMessage(sessionId, result);
      } catch (error) {
        logger.warn?.('vision-bridge: admission probe cleanup failed (' + truncateText(error?.message ?? error) + ')');
      }
    }
    return { runtime: 'dead' };
  } catch (error) {
    if (classifyAdmissionProbeError(error) === 'live') return { runtime: 'live', reason: ADMISSION_GATE_LIVE_REASON };
    logger.warn?.('vision-bridge: admission probe inconclusive (' + truncateText(error?.message ?? error) + ')');
    return { runtime: 'unknown', reason: 'inconclusive' };
  }
}
...
export function installServerRoutes(scope, deps = {}) {
  ...
  const getSeamState = deps.getSeamState;
  ...
  /** §4.7-2/§4.6: the runtime seam state, normalized for JSON. `live` is
   * tri-state — true / false / null ("not self-proven yet") — and a missing or
   * throwing source degrades to unknown instead of failing the health route. */
  function readSeamState() {
    let raw;
    try {
      raw = typeof getSeamState === 'function' ? getSeamState() : undefined;
    } catch (error) {
      logger.warn?.('vision-bridge: runtime seam state unavailable (' + truncateText(error?.message ?? error) + ')');
      raw = undefined;
    }
    const state = raw ?? {};
    return {
      ...
      include: Array.isArray(state.include) ? state.include : [],
      requireReader: typeof state.requireReader === 'boolean' ? state.requireReader : null,
      // F3/N-B2-4: a real residue (mark present, but the method is not our
      // layer) — a boolean fact, never inferred from `live` by the card.
      residue: state.residue === true,
    };
  }

  /** GET /vision-bridge/env — providers projection + environment checks.
   * The admission item is two-faced since §12: `disk` is the marker state
   * (unchanged semantics) and `runtime` is the behavior probe's verdict about
   * the gate the running process actually holds. `conflict` marks the §12.1
   * story-1 situation — disk repaired, running gate still live. */
  async function handleEnv(req, res) {
    ...
    try {
      ...
      const disk = admission.status === 'ok' ? 'patched' : admission.status === 'missing' ? 'missing' : 'unknown';
      const seam = readSeamState();
      // §4.7-2: while the seam is live the synthetic image prompt WOULD be
      // admitted (that is what the seam does), so the probe can only ever read
      // 'dead' — a verdict about the seam, not about the loaded gate. Report the
      // distinct third state and submit nothing at all (no scratch residue).
      const probe = seam.live === true
        ? { runtime: 'skipped', reason: ADMISSION_PROBE_SKIPPED_REASON }
        : await probeAdmissionGate({
          logger,
          promptRemote: deps.promptRemote,
          ensureScratchSession: deps.ensureScratchSession,
          cleanupScratchMessage: deps.cleanupScratchMessage,
      ...
      sendJson(res, 200, {
        ...
        admission: {
          // §11 legacy fields (the card's first generation and its tests read them)
          status: admission.status,
          files: admission.files.map((f) => ({ file: f.file, status: f.status })),
          // §12 two-faced admission state
          disk,
          runtime: probe.runtime,
          reason: probe.reason,
          conflict: disk === 'patched' && probe.runtime === 'live',
        },
        // §4.6/§4.7: the runtime seam state the card renders (read-only).
        seam,
        modlens,
      });
    } catch (error) {
