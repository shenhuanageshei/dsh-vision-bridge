 ...
 */
export function projectProviders(settingsGet) {
  let raw;
  try {
...
/** Whether a same-id modlens override row (pasteToPath: false) already exists. */
export function findModlensOverride(text) {
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    for (let j = i + 1; j < lines.length; j++) {
      ...
      if (/^\s*pasteToPath:\s*false\s*(?:#.*)?$/.test(lines[j])) return true;
    }
  }
  return false;
}
...
/** The document text with the modlens override row appended. */
...
export function installServerRoutes(scope, deps = {}) {
  ...
  async function handleEnv(req, res) {
    ...
    try {
      const providers = projectProviders(settingsGet);
      let config;
      try {
        config = safeConfig();
      } catch {
