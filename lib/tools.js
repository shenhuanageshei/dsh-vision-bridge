  ...
  return defineTool({
    ...
    async execute(args, exec) {
      ...
      try {
        ...
        throw new Error(error.message + ' Available images: ' + formatCandidates([...index.values()].slice(0, 8)));
      }
      ...
      if (found.status === 'not-found') {
        throw new Error('vision_bridge_read: no session image matches ref "' + args.ref + '". Available images: ' + formatCandidates([...index.values()].slice(0, 8)) + ' (pass the 8-hex id from the "[image omitted …]" placeholder, or "latest")');
      }
