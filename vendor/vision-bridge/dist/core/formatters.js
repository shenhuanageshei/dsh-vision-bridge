 ...
 */
export function safeSection(value, fallback = 'none') {
 ...
 */
export function truncate(text, max = MAX_NOTE_CHARS) {
 ...
 */
export function normalizeBox(rawBox, capabilities) {
 ...
 */
export function normalizePrimitive(raw, index, capabilities) {
    const boxesEnabled = capabilities?.boxes !== false;
    const pointsEnabled = capabilities?.points !== false;
    const ref = String(raw.ref ?? raw.label ?? raw.text ?? raw.name ?? `element-${index}`);
    const id = String(raw.id ?? `v${index + 1}`);
    const grounding = raw.grounding ?? capabilities?.groundingMode ?? 'native';
    const confidence = normalizeConfidence(raw.confidence);
    // prefer box over point
    if (boxesEnabled) {
        const boxRaw = raw.box ?? raw.bbox ?? raw.bbox_2d ?? raw.box_2d;
 ...
 */
export function formatVisualPrimitives(primitives, groundingMode) {
    const lines = [
    ...
    if (!primitives.length) {
        ...
        for (const p of limited) {
            ...
            if (p.box)
                parts.push(`box: [${p.box.join(', ')}]`);
            if (p.point)
                parts.push(`point: [${p.point.join(', ')}]`);
            const refText = p.ref.length > MAX_PRIMITIVE_REF_CHARS
 ...
 */
export function formatStructuredVisionNote(analysis, capabilities) {
    const sections = [
    ...
    const rawPrimitives = analysis.visual_primitives ??
 ...
 */
export function formatInvalidStructuredNote(rawResponse) {
    const lines = [
